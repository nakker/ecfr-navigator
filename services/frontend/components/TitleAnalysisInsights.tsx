...
interface TitleAnalysisInsightsProps {
  titleNumber: number;
}
...
export default function TitleAnalysisInsights({ titleNumber }: TitleAnalysisInsightsProps) {
  ...
  const { 
    ...
    ['antiquatedSections', titleNumber],
    async () => {
      const response = await axios.get(`/api/analysis/antiquated?limit=5&titleNumber=${titleNumber}`);
  ...
  const { 
    ...
    ['businessUnfriendlySections', titleNumber],
    async () => {
      const response = await axios.get(`/api/analysis/business-unfriendly?limit=5&titleNumber=${titleNumber}`);
      return response.data as AnalyzedSection[];
    },
    {
      staleTime: 10 * 60 * 1000, // Consider data stale after 10 minutes
  ...
  const handleNavigate = (section: AnalyzedSection) => {
    console.log('TitleAnalysisInsights - handleNavigate called with:', section);
    console.log('Current router:', { pathname: router.pathname, query: router.query });
    
    // Use the identifier from the populated documentId object, fallback to sectionIdentifier
    const sectionIdentifier = section.documentId?.identifier || section.sectionIdentifier || section.identifier;
    console.log('Using section identifier:', sectionIdentifier);
    console.log('Full section data:', {
    ...
    if (!sectionIdentifier) {
      console.error('No valid identifier found for section:', section);
      return;
    }
    
    // If we're already on the correct title page, just update the hash
    if (router.pathname === '/title/[number]' && router.query.number === String(section.titleNumber)) {
      // Update the hash directly
      window.location.hash = sectionIdentifier;
      
      // Trigger a custom event to notify DocumentsList to open the section
      window.dispatchEvent(new CustomEvent('navigateToSection', { 
        detail: { sectionId: sectionIdentifier } 
      }));
    } else {
      // Navigate to the title page with the section hash
      router.push(`/title/${section.titleNumber}#${sectionIdentifier}`);
    }
  };
      ...
      <Box sx={{ mt: 4, mb: 4 }}>
        ...
        <Alert severity="info" sx={{ mb: 3 }}>
          <Typography variant="body2">
            AI analysis is pending for this title. Sections will be analyzed during the next processing cycle.
  ...
  return (
    <Box sx={{ mt: 4, mb: 4 }}>
      <Typography variant="h5" sx={{ mb: 3, fontWeight: 600 }}>
        Regulatory Analysis Insights for Title {titleNumber}
      </Typography>
          ...
          {antiquatedError ? (
            ...
            <Card elevation={2}>
              <CardContent>
                ...
                <Box sx={{ py: 4, textAlign: 'center' }}>
                  <Typography variant="body2" color="text.secondary">
                    No outdated sections found for this title
                  </Typography>
                </Box>
              </CardContent>
            </Card>
          ...
          {businessError ? (
            ...
            <Card elevation={2}>
              <CardContent>
                ...
                <Box sx={{ py: 4, textAlign: 'center' }}>
                  <Typography variant="body2" color="text.secondary">
                    No high business burden sections found for this title
                  </Typography>
                </Box>
              </CardContent>
            </Card>
